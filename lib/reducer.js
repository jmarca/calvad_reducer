...
function reducer(opts){
    ...
    function process_collated_record(record){
        ...
        _.each([vo.n
               ...
               ,vo.not_heavyheavy]
              ,function(i){
                   _stash[tkey][fkey].data[i]+=(nums[i]*l)
               });
        // the m vars need to be multiplied by the proper VMT value
        _stash[tkey][fkey].data[vo.o]          += nums[vo.o] * nums[vo.n]*l
        _stash[tkey][fkey].data[vo.hh_weight]  += nums[vo.hh_weight] * nums[vo.heavyheavy]*l
        _stash[tkey][fkey].data[vo.hh_axles]   += nums[vo.hh_axles]  * nums[vo.heavyheavy]*l
        _stash[tkey][fkey].data[vo.nhh_weight] += nums[vo.nhh_weight]* nums[vo.not_heavyheavy]*l
        _stash[tkey][fkey].data[vo.nhh_axles]  += nums[vo.nhh_axles] * nums[vo.not_heavyheavy]*l

        // ditto the hm vars, but they have to be harmonically meaned
        _stash[tkey][fkey].data[vo.hh_speed] += nums[vo.heavyheavy]*l/nums[vo.hh_speed]
        _stash[tkey][fkey].data[vo.nhh_speed] += nums[vo.not_heavyheavy]*l/nums[vo.nhh_speed]
        _stash[tkey][fkey].data[vo.wgt_spd_all_veh_speed] += nums[vo.n]*l/nums[vo.wgt_spd_all_veh_speed]

        _stash[tkey][fkey].len += l
        _stash[tkey][fkey].lanemiles += l * record.lanes
        //if(_.indexOf(_stash[tkey][fkey].detectors,record.detector_id) === -1 ){
                       ...
                       _.each(freeways
                             ,function(fkey){
                                  if( _stash[tkey][fkey] !== undefined ){
                                      ...
                                      _stash[tkey][fkey].detectors = _.unique(_stash[tkey][fkey].detectors,true)
